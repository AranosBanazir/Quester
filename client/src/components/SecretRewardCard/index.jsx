...
const SecretRewardCard = ({ reward, onRedeem, onDelete }) => {
  ...
  const handleRedeemClick = async () => {
    if (
      window.confirm(
        `Are you sure you want to redeem the reward "${reward.name}"?`
      )
    ) {
      try {
  ...
  const handleDeleteClick = async () => {
    if (
      window.confirm(
        `Are you sure you want to delete the reward "${reward.name}"? This action cannot be undone.`
      )
    ) {
      try {
  ...
  return (
    <div className="card card-compact bg-base-100 w-96 shadow-xl">
      <figure>
        <img src="/rewardstar.png" alt="reward star" />
      </figure>
      ...
      <div className="card-body">
        ...
        <div className="card-actions justify-end">
          <button
            className="btn btn-success self-end mt-5"
            onClick={handleRedeemClick}
          >
            <p className="mr-2">Cost: {reward.cost}</p>
          </button>
          <button
            className="btn btn-error self-end mt-5"
