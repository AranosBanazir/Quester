...
const ParentTasks = () => {
  ...
  const [addTask, { data: taskData, loading: taskLoading, error: taskError }] =
  ...
  if (userError)
    return (
      <p className="text-center text-red-500">
        Error loading user data: {errorHandler(userError.message)}
      </p>
    );
  if (tasksError)
    return (
      <p className="text-center text-red-500">
        Error loading tasks: {errorHandler(tasksError.message)}
      </p>
    );
    ...
    <div className="container mx-auto p-6">
      <section className="bg-gray-800 text-white p-6 rounded-md shadow-lg mb-6">
        <form onSubmit={handleTaskSubmit}>
          ...
          <div className="mb-2">
            <label
              className="block text-sm font-medium text-gray-300 mb-1 permanent-marker-regular"
              htmlFor="taskName"
            >
              Name:
            </label>
          ...
          {taskError && (
            <p className="mt-2 text-red-500 text-sm">
              Error:{" "}
              {errorHandler(taskError.message) || "An unknown error occurred"}
            </p>
          )}
        </form>
      </section>
